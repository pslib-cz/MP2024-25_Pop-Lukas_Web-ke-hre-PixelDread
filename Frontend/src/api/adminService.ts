...
export const updateAdmin = async (id: string, adminData: { email?: string; password?: string }): Promise<Admin> => {
  const response = await axiosInstance.put(`/Admin/UpdateAdmin/${id}`, adminData);
  return response.data;
};
